# Last updated at 2017-07-02 20:43:50.247701613 -0700 PDT
...
RUN pushd ${GOPATH}/src/github.com/gyuho/deephardway \
  ...
  && which node \
  && node -v \
  && cp /usr/local/nvm/versions/node/v8.1.3/bin/node /usr/bin/node \
  && popd
##########################
...
RUN cat /etc/lsb-release >> /container-version.txt \
  ...
  && printf "\n" >> /container-version.txt \
  && echo NPM: $(/usr/local/nvm/versions/node/v8.1.3/bin/npm --version 2>&1) >> /container-version.txt \
  && printf "\n" >> /container-version.txt \
