# Last updated at 2017-08-16 09:07:21.48986527 -0700 PDT
...
RUN cat /etc/lsb-release >> /container-version.txt \
  ...
  && printf "\n" >> /container-version.txt \
  && echo NPM: $(/usr/local/nvm/versions/node/v8.4.0/bin/npm --version 2>&1) >> /container-version.txt \
  && printf "\n" >> /container-version.txt \
