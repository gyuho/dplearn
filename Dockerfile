# Last updated at 2017-06-22 10:04:29.940714572 -0700 PDT
...
RUN cat /etc/lsb-release >> /container-version.txt \
  ...
  && printf "\n" >> /container-version.txt \
  && echo NPM: $(/usr/local/nvm/versions/node/v8.1.2/bin/npm --version 2>&1) >> /container-version.txt \
  && printf "\n" >> /container-version.txt \
